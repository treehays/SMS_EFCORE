...
public class AdminManager : IAdminManager
{
    ...
    public void CreateAdmin(int id, string firstName, string lastName, string email, string phoneNumber, string pin, string post)
    {
        var staffId = User.GenerateRandomId();
        var admin = new Admin(id, staffId, firstName, lastName, email, phoneNumber, pin, post);
        try
        {
        ...
        try
        {
            using (var connection = new MySqlConnection(ConnString))
            {
                ...
                using (var command = new MySqlCommand($"select * From admin WHERE staffId = '{staffId}'", connection))
                {
                    ...
                    while (reader.Read())
                    {
                        admin = new Admin(int.Parse(reader["staffId"].ToString()),reader["staffId"].ToString().ToUpper(), reader["firstName"].ToString(), reader["lastName"].ToString(), reader["email"].ToString(), reader["phonenumber"].ToString(), reader["Pin"].ToString(), reader["post"].ToString());
                    }
                }
            }
        }
        ...
        try
        {
            using (var connection = new MySqlConnection(ConnString))
            {
                ...
                using (var command = new MySqlCommand($"select * From attendant WHERE email = '{email}'", connection))
                {
                    ...
                    while (reader.Read())
                    {
                        admin = new Admin(int.Parse(reader["staffId"].ToString()),reader["staffId"].ToString(), reader["firstName"].ToString(), reader["lastName"].ToString(), reader["email"].ToString(), reader["phonenumber"].ToString(), reader["Pin"].ToString(), reader["post"].ToString());
                    }
                }
            }
        }
        ...
        try
        {
            using (var connection = new MySqlConnection(ConnString))
            {
                ...
                using (var command = new MySqlCommand($"select * From admin WHERE StaffId = '{staffId}'", connection))
                {
                    ...
                    while (reader.Read())
                    {
                        admin = new Admin(int.Parse(reader["staffId"].ToString()),reader["staffId"].ToString(), reader.GetString(2), reader["lastName"].ToString(), reader["email"].ToString(), reader["phonenumber"].ToString(), reader["Pin"].ToString(), reader["post"].ToString());
                    }
                }
            }
        }
