...
public class AttendantManager : IAttendantManager
{
    ...
    public void CreateAttendant(int id, string firstName, string lastName, string email, string phoneNumber, string pin, string post)
    {
        var staffId = User.GenerateRandomId();
        var attendant = new Attendant(id,staffId, firstName, lastName, email, phoneNumber, pin, post);
        try
        {
        ...
        try
        {
            using (var connection = new MySqlConnection(ConnString))
            {
                ...
                using (var command = new MySqlCommand($"SELECT * From attendant WHERE staffId = '{staffId}'", connection))
                {
                    ...
                    while (reader.Read())
                    {
                        attendant = new Attendant(int.Parse(reader["staffId"].ToString()),reader["staffId"].ToString().ToUpper(), reader["firstName"].ToString(), reader["lastName"].ToString(), reader["email"].ToString(), reader["phonenumber"].ToString(), reader["Pin"].ToString(), reader["post"].ToString());
                    }
                }
            }
        }
        ...
        try
        {
            using (var connection = new MySqlConnection(ConnString))
            {
                ...
                using (var command = new MySqlCommand($"SELECT * From attendant WHERE email = '{email}'", connection))
                {
                    ...
                    while (reader.Read())
                    {
                        attendant = new Attendant(int.Parse(reader["staffId"].ToString()),reader["staffId"].ToString(), reader["firstName"].ToString(), reader["lastName"].ToString(), reader["email"].ToString(), reader["phonenumber"].ToString(), reader["Pin"].ToString(), reader["post"].ToString());
                    }
                }
            }
        }
